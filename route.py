...
class Route():
    ...
    def kan_ik_tot_hier(self, eind_x, eind_y, game, eenheid):
        global nieuwe_lijst_met_tegels, veld_met_tegels, terijn
        lijst_met_tegels = []
        begin_x = eenheid.cordinaten.x
        begin_y = eenheid.cordinaten.y
        ...
        if self.kortse_weg[BEWEGEN] < 20:
            ...
            if eenheid.boot == 1:
                self.kortse_weg[UITHOUDING] = uithouding_terijn[terijn[kortse_weg[Y]][kortse_weg[X]]]
                self.kortse_weg[UITHOUDING] -= uithouding_terijn[terijn[begin_y][begin_x]]
                ...
                self.kortse_weg[UITHOUDING] = uithouding_terijn_boot[terijn[kortse_weg[Y]][kortse_weg[X]]]
                self.kortse_weg[UITHOUDING] -= uithouding_terijn_boot[terijn[begin_y][begin_x]]
                while len(vorige_tegel) >= 4:
                    kortse_weg[UITHOUDING] += uithouding_terijn_boot[terijn[vorige_tegel[Y]][vorige_tegel[X]]]
                    game.highlight((vorige_tegel[X] + game.begin_teken_x) * 16, (vorige_tegel[Y] + game.begin_teken_y) * 16)
                    vorige_tegel = vorige_tegel[VORIGE]
            # print(kortse_weg[4])
            return self.kortse_weg
        else:
            # print('False')
            return False


    def rondom_kijken(self, begin_x, begin_y, begin_bewegen, vorige, boot, game):
        # print(vorige)
        for x in range(-1, 2):
            for y in range(-1, 2):
                xPos = begin_x + x
